...
class ActLinear(nn.Module):
    ...
    def forward(self, x):
        if self.record_activation:
            ...
            # print ("x",x.shape,x_.shape,x_.view(-1, x_.shape[-1]).cpu().shape)
            # print ("self.activation_norms",self.activation_norms[-1].shape,len(self.activation_norms))
...
def make_low_rank(
    args, rank_pruning,model, tokenizer, device=torch.device("cuda:0"), prune_data="wikitext"
):
    ...
    for layer in range(num_hidden_layers):
        ...
        for name, module in model.named_modules():
            if layer_filter_fn(name) and isinstance(module, ActLinear):
                ...
                # print ("module.activation_norms",len(module.activation_norms), (module.activation_norms[0]).shape,(module.activation_norms[1]).shape,(module.activation_norms[2]).shape)
                ...
                # print ("module.activation_norms",module.activation_norms.shape)
                score = (
                    module.activation_norms @ module.base.weight.data.T
                )  # (size * d_in) @ (d_out * d_in).T --> (size, d_out)

                # print ("module.base.weight.data.T",module.base.weight.data.T.shape)
                ...
                if match:
                    layer_num, layer_name = match.groups()

                # pdb.set_trace()
                args.rank=rank_pruning[int(layer_num)][layer_name]


                print  ("sparse.rank",args.rank)


                print(
                    f"remaining: rank {name} = {total_rank - args.rank} / {total_rank},",
                    f"sparsity: rank {name} = {args.rank / total_rank}"
                )
